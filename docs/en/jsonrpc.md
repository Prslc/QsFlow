...
| `action:<desktop-id>:<action-id>` | run a desktop action; the UI resolves it through Quickshell's desktop-entry model (app-search emits one row per action, DMS-style) |
