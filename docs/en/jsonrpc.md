...
| `action:<desktop-id>:<action-id>` | run a desktop action; the shell forwards it as `action <desktop-id>:<action-id>` and the core resolves the `[Desktop Action …]` group itself (app-search emits one row per action, DMS-style) |
